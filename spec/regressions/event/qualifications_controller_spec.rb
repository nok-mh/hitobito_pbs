...
describe Event::QualificationsController, type: :controller do
  ...
  before do
    role = Fabricate(:event_role, type: Event::Course::Role::Participant.name)
    Fabricate(:event_participation, person: bulei, event: course, roles: [role], active: true)
    course.update(qualifies: true)
    sign_in(advisor)
  end
  ...
  describe 'GET index' do
    ...
    context 'advisor / LKB' do
      ...
      it 'can view qualifications' do
        ...
        expect(page).to have_css('td.issue a i.disabled')
        expect(page).to have_css('td.revoke a i.disabled')
